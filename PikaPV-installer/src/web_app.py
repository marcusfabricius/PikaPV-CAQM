...
import pikapv_backend as backend
...
def installed_data_dir() -> Path:
    ...
    return root / "CAQM" / "PikaPV"


DEFAULT_DATA_DIR = installed_data_dir() if IS_FROZEN else SOURCE_DIR
BASE_DIR = Path(os.environ.get("PIKAPV_DATA_DIR", DEFAULT_DATA_DIR)).expanduser().resolve()
DEFAULT_SETTINGS_FILE = BASE_DIR / "default_settings.yaml"
SPEED_PROFILE_SETTINGS_FILE = BASE_DIR / "speedprofile_settings.yaml"
DEFAULT_SETTINGS_TEMPLATE = RESOURCE_DIR / "default_settings.yaml"
SPEED_PROFILE_SETTINGS_TEMPLATE = RESOURCE_DIR / "speedprofile_settings.yaml"
LOG_FILE = BASE_DIR / "logs" / "pikapv.log"
...
def initialize_data_files() -> None:
    ...
    for destination, template in (
        (DEFAULT_SETTINGS_FILE, DEFAULT_SETTINGS_TEMPLATE),
        (SPEED_PROFILE_SETTINGS_FILE, SPEED_PROFILE_SETTINGS_TEMPLATE),
    ):
        if destination.exists() or destination.resolve() == template.resolve():
            continue
...
def settings_from_payload(payload: Dict[str, Any]) -> Any:
    ...
    if not settings.output_dir.is_absolute():
        settings.output_dir = BASE_DIR / settings.output_dir
    return settings
...
if __name__ == "__main__":
    ...
    try:
        ...
        terminal_log("Serving PikaPV with Waitress on 127.0.0.1:5000")
        serve(app, host="127.0.0.1", port=5000, threads=6)
    except ImportError:
        terminal_log("WARNING: Waitress is unavailable; using Flask development server.")
